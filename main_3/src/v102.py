...
def augment_image(image, label):
    # 1. 旋转 (使用 tf.keras.layers.RandomRotation 替代 tfa.image.rotate)
    # 旋转角度为 [-pi/6, pi/6] (即 [-30, 30] 度)
    # factor = (pi/6) / (2*pi) = 1/12
    image = layers.RandomRotation(factor=1/12, interpolation='bilinear')(image, training=True)
    ...
    # 8. 随机平移 (使用 tf.keras.layers.RandomTranslation 替代 tfa.image.translate)
    # 平移范围为高度/宽度的 +/- 10%
    # height_factor = (IMG_HEIGHT // 10) / IMG_HEIGHT = 0.1
    # width_factor = (IMG_WIDTH // 10) / IMG_WIDTH = 0.1
    image = layers.RandomTranslation(height_factor=0.1, width_factor=0.1, interpolation='bilinear')(image, training=True)
