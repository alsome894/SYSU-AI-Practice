...
NUM_CLASSES = 9  # 类别数量 (后续会根据数据动态调整)
...
class TripletAttention(layers.Layer):
    ...
    def call(self, x):
        ...
        # 通道-高度注意力分支 (C-H)
        x_ch_perm = tf.transpose(x, perm=[0, 3, 1, 2])  # 调整维度顺序以沿宽度轴池化 (N, C, H, W)
        max_p_ch = tf.reduce_max(x_ch_perm, axis=3, keepdims=True)  # 宽度轴最大池化 (N, C, H, 1)
        avg_p_ch = tf.reduce_mean(x_ch_perm, axis=3, keepdims=True)  # 宽度轴平均池化 (N, C, H, 1)
        z_p_ch = tf.concat([max_p_ch, avg_p_ch], axis=3)  # 拼接池化结果 (N, C, H, 2)
        attn_ch_map_intermediate = self.ch_conv(z_p_ch)  # 生成通道-高度注意力图 (N, C, H, 1)
        attn_ch_map_perm = tf.transpose(attn_ch_map_intermediate, perm=[0, 2, 3, 1])  # 调整维度以匹配输入 (N, H, 1, C)
        out_ch = x * attn_ch_map_perm  # 加权特征图

        # 通道-宽度注意力分支 (C-W)
        x_cw_perm = tf.transpose(x, perm=[0, 3, 2, 1])  # 调整维度顺序以沿高度轴池化 (N, C, W, H)
        max_p_cw = tf.reduce_max(x_cw_perm, axis=3, keepdims=True)  # 高度轴最大池化 (N, C, W, 1)
        avg_p_cw = tf.reduce_mean(x_cw_perm, axis=3, keepdims=True)  # 高度轴平均池化 (N, C, W, 1)
        z_p_cw = tf.concat([max_p_cw, avg_p_cw], axis=3)  # 拼接池化结果 (N, C, W, 2)
        attn_cw_map_intermediate = self.cw_conv(z_p_cw)  # 生成通道-宽度注意力图 (N, C, W, 1)
        attn_cw_map_perm = tf.transpose(attn_cw_map_intermediate, perm=[0, 3, 2, 1]) # 调整维度以匹配输入 (N, 1, W, C)
...
def build_triplet_efficientnet_b7(input_shape=(IMG_HEIGHT, IMG_WIDTH, 3), num_classes=NUM_CLASSES, triplet_kernel_size=TRIPLET_KERNEL_SIZE):
    ...
    for layer_idx, layer in enumerate(base_model.layers[1:]):  # 跳过输入层
        ...
        for node_obj in layer._inbound_nodes: # 一个层可能有多个入站节点
        ...
        if isinstance(layer, tf.keras.layers.Multiply) and "se_excite" in layer.name:
            if not isinstance(current_input_tensor_for_layer, list) or len(current_input_tensor_for_layer) != 2:
                ...
                x = attention_output # TripletAttention直接输出加权特征图
        else:
            x = layer(current_input_tensor_for_layer)
    ...
    # 使用传入的 num_classes (即动态调整后的 NUM_CLASSES)
...
def parse_image(filename):
    ...
    image = tf.image.decode_jpeg(image, channels=3) # 输出 uint8 张量
    image = tf.image.resize(image, [IMG_HEIGHT, IMG_WIDTH]) # 输出 float32 张量
    return image # 返回的 image 是 float32 [0,255] 范围，如果 decode_jpeg 后直接 resize

def augment_image(image, label):
    """
    对图像应用九种数据增强技术。
    输入 'image' 预期为 tf.float32 类型，范围在 [0,1]。
    """
...
def load_dataset(subset_dir, class_names_list_arg):
    ...
    参数 class_names_list_arg 是实际的类别名称列表。
    """
    ...
    # 使用 class_names_list_arg 的长度，它代表了从数据中实际检测到的类别数
...
if __name__ == '__main__':
    ...
    if gpus:
        try:
            ...
            print("将继续执行，但可能导致TensorFlow占用全部GPU显存。")
    else:
        print("未找到可用的GPU。TensorFlow 将使用 CPU 进行训练。")
        print("如果您拥有NVIDIA GPU，请确保已正确安装CUDA、cuDNN，并且您的TensorFlow版本支持GPU。")
    ...
    if len(class_names_from_data) != NUM_CLASSES:
         ...
         NUM_CLASSES = len(class_names_from_data) # 更新全局NUM_CLASSES
    ...
    print("正在加载数据集...")
    try:
        # load_dataset 现在会正确使用 CLASS_NAMES_LIST 的长度进行 one-hot 编码
    ...
    if train_dataset_cardinality > 0 and train_dataset_cardinality < 1000: # 避免 buffer_size 大于数据集大小
    ...
    model.summary(line_length=150) # 增加行长度以便更好地显示
    ...
    if final_y_true is not None and len(final_y_true) > 0:
        print("\n分类报告：")
        # 确保 CLASS_NAMES_LIST 用于 target_names
    ...
    # plt.show() # Consider commenting out if running in a non-interactive environment
