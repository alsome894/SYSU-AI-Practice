# TECHNIQUE: Gradual Unfreezing (Simplified: Two-Stage Training)
# Stage 1: Train only the classifier head.
# Stage 2: Unfreeze all layers and continue training with a lower learning rate.
...
def train_model(model, criterion, optimizer, scheduler, dataloaders, dataset_sizes, device, num_epochs=25, current_epoch_offset=0): # Added offset for combined history
    since = time.time()
    best_model_wts = copy.deepcopy(model.state_dict()) # Best model weights tracked across all stages from the initial call
    best_acc = 0.0 # Best accuracy tracked across all stages
    
    # history dictionary should be initialized outside if combining history across stages
    # For simplicity here, each call to train_model will return its own history.
    # A more robust implementation would pass and update a single history dict.
    ...
    # model.load_state_dict(best_model_wts) # Load best model weights at the very end of all stages
...
def main_gradual_unfreezing():
    ...
    data_transforms = { # Basic augmentations
    ...
    for param in model_ft.classifier[1].parameters(): # Unfreeze only the new head
    ...
    for param in model_ft.parameters(): # Unfreeze all layers
        param.requires_grad = True
        
    optimizer_stage2 = optim.AdamW(model_ft.parameters(), lr=lr_stage2) # Optimizer for all parameters
    scheduler_stage2 = optim.lr_scheduler.StepLR(optimizer_stage2, step_size=5, gamma=0.1) # Or CosineAnnealingLR for fine-tuning
    
    # Pass current best weights and accuracy to continue tracking
    model_ft.load_state_dict(best_model_wts_overall) # Start stage 2 from best model of stage 1
    ...
    # ... (Evaluation and saving as in Script 1, using model_name_suffix) ...
